...
#把生日年的部分替替換 str_replace pattern 2020
```{r}
#一開頭^的4個數字(年分)(4條0-9的捲帶)替換成2020
str_bdates<-
  str_replace(
    date(birthday_list$date),
    "^[0-9]{4}",
...
```
```{r}
class(str_bdates)#使用str的fn,回傳的值都會是character
```
```{r}
#要cut 他
bdate<-lubridate::ymd(str_bdates) 
...
str_sign_number<-
  ...
  horoscopes,
  "([:graph:]{2}座|[0-9]+)"
)#字組捲帶(pharase1|pharase2)
#pharase2 找出連著寫的數字,不管幾串
#pharase1 :graph:包含所有文字xx座

```
```{r}
str_sign_number[[12]]
str_sign_number[[12]][[2]]
str_sign_number[[12]][[3]]
str_sign_number[[12]]
```
2020-2-20
```{r}
...
#.x=1:12
breakmiddlemapping<-{unlist(map(1:12,
 ~{paste0(
  "2020-",
  str_sign_number[[.x]][[2]],"-",
  str_sign_number[[.x]][[3]]
)}   ))->str_sign_beginingdate
ymd(str_sign_beginingdate)}
```
cut
```{r}
#2020 1/1-12/31+1(手動加)/Inf
#[1/21-2/20)水瓶座
#[2/20-3/20)雙魚......12刀
cut(
  bdate,
...
```
```{r}
horoscopes_for_allBreaks<-unlist(map(
  str_sign_number,~{
    .x[[1]]
  }
  ))
