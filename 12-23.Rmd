...
#抽出使用者訊息
#輸入memberIds,回傳資料
# takes the sampled memberID and currentGPS as input arguments and return a list of 7 representing this user.
#currentGpS<-latestGPS
currentGPS<-map(
  seq_along(members),
~{members[[.x]]$latestGPS}
)
...
memberIds<-function(members){
  
}
appLogin<-function(memberID, currentGPS){
  #輸入memberIds,
  #currectGPS 來更新會員資料裡的latestGPS
 currentGPS<-map(
  seq_along(members),
~{members[[.x]]$latestGPS}
) 
